...
$(document).ready(function (){
  ...
  $('#button').click(function () {
    ...
    var type_icon, currency_icon; //Store the html fragment to a variable when needed.
    if ($type.val() == 'card') {
    ...
    if ($currency.val() == 'dollar') {
      currency_icon = "$";
    } else if ($currency.val() == 'rupee') {
      currency_icon = "&#x20b9;";
    } else if ($currency.val() == 'euro') {
      currency_icon = "&pound;";
    };
    ...
    if(form_validated) {
      ...
      $("table tr:first").after('<tr><td>'+type_icon+'</td><td>'+$name.val()+'</td><td>'+$friend.val()+'</td><td>'+formatDate($date.val())+'</td><td class="amount">'+currency_icon+''+$amount.val()+'</td><td><i class="fa fa-pencil"></i></td><td><i class="fa fa-trash-o"></i></td></tr>');
      $date.val(null);
      $amount.val(null);
      $name.val(null);
      $("#if-empty").remove();
